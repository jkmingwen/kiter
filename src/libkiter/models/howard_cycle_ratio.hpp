...
#define VERBOSE_HOWARD(msg)   VERBOSE_CUSTOM_DEBUG ("HOWARD", msg)
 ...
 * @updated by Bruno Bodin
 */


#define EPSILON_IMPROVE_RATIO 100
...
namespace boost {
  ...
  namespace detail {
    ...
    public:
      ...
      mcr_howard(const Graph &g, VertexIndexMap vim,
        ...
        m_colcv(m_V),
        m_col_bfs(m_V)
      {VERBOSE_HOWARD("m_bound=" << m_bound << " m_cr=" << m_cr);}
       ...
       * \param ewm  edge weight map. Read property map: E -> R
       * \param ew2m  edge weight map. Read property map: E -> R+
       * \param bound The custom bound.
       */
      mcr_howard(const Graph &g, VertexIndexMap vim,
                  EdgeWeight1 ewm, EdgeWeight2 ew2m, typename FloatTraits::value_type bound) :
        m_g(g), m_vim(vim), m_ew1m(ewm), m_ew2m(ew2m),
        m_bound(bound),
        m_cr(m_bound),
        m_V(num_vertices(m_g)),
      ...
      float_t ocr_howard()
      {
        ...
        if ( k >= MAX_ITERATION) {
            ...
            VERBOSE_HOWARD("HOWARD FINSH at iteration " << k << " / " << MAX_ITERATION);
        }

        const float_t eps_ =  -FloatTraits::epsilon() * cmp_props_t::multiplier;

        if (m_cmp(mcr, mcr_bound() + eps_))
          {
        	VERBOSE_ERROR("UNKNOWN HOWARD ERROR : mcr = " << mcr << " bound = " << m_bound  << " first bound = " << mcr_bound());
        	//VERBOSE_FAILURE();
    ...
    protected:
      virtual void store_critical_edge(edge_t, critical_cycle_t &) {VERBOSE_FAILURE();}
      virtual void store_critical_cycle(critical_cycle_t &)        {VERBOSE_FAILURE();}
    ...
    private:
      ...
      {
    	VERBOSE_HOWARD("Call cycle_ratio");
        if (sv == m_sink) {
        	VERBOSE_HOWARD("  sink");
        	return m_bound;
        }
        ...
        do
          {
        	VERBOSE_HOWARD("  store edge " << m_policy[v] << " in local cc");
    	...
    	VERBOSE_HOWARD("  Compare m_cr and cr " << m_cr << "->" << cr);
        if ( m_cmp(m_cr, cr) )
          {
            m_cr = cr;
            VERBOSE_HOWARD(" store a new critical cycle with m_cr = " << cr);
      ...
      float_t policy_mcr()
      {
    	VERBOSE_HOWARD("policy_mcr() : Begin");
        ...
        while ( (uv_itr = std::find_if(uv_itr, vie,
          ...
          ///While there are undiscovered vertices
          {
            ...
            VERBOSE_HOWARD(" =>  find cycle =" << gv);
            float_t cr = cycle_ratio(gv) ;
            VERBOSE_HOWARD(" =>  new CR =" << cr);
            mcr_bfv(gv, cr, vcm_);
            if ( m_cmp(mcr, cr) )  {
            	VERBOSE_HOWARD(" =>  Update MCR " << mcr << "->" << cr);
            	mcr = cr;
            }
            ++uv_itr;
          }
        VERBOSE_HOWARD("policy_mcr() : End, mcr =" << mcr);
        return mcr;
      }
      ...
      /*!
          ...
          */
         bool check_improve_policy(float_t cr)
         {
           bool improved = false;
           typename  graph_traits<Graph>::vertex_iterator  vi, vie;
           typename  graph_traits<Graph>::out_edge_iterator  oei, oeie;
                   ...
                   for (boost::tie(oei, oeie) = out_edges(*vi, m_g); oei != oeie; ++oei)
                     {
                       ...
                       if ( m_cmp(m_dm[*vi] - eps_ * EPSILON_IMPROVE_RATIO , dis_) )
                         {
                    	   //VERBOSE_ERROR("Successfull compared " << m_dm[*vi] << " and " << dis_);
  ...
  * \brief Finds optimum cycle raio and a critical cycle
  ...
  optimum_cycle_ratio(const TG &g, TVIM vim, TEW1 ewm, TEW2 ew2m, EV* pcc,  typename FT::value_type bound )
     {
       ...
       detail::mcr_howard1<FT, TG, TVIM, TEW1, TEW2> obj(g, vim, ewm, ew2m, bound);
       typename FT::value_type ocr = obj.ocr_howard();
...
// Maximum Cycle Ratio
  ...
  inline typename FloatTraits::value_type
  maximum_cycle_ratio(const Graph &g, VertexIndexMap vim, EdgeWeight1Map ew1m,
                      EdgeWeight2Map ew2m,  typename FloatTraits::value_type bound,
                      std::vector<typename graph_traits<Graph>::edge_descriptor>* pcc = 0,
                      FloatTraits = FloatTraits())
  {
      typedef detail::float_wrapper<
          FloatTraits, detail::max_comparator_props<FloatTraits>
      > Traits;
      return detail::optimum_cycle_ratio<Traits>(g, vim, ew1m, ew2m, pcc,bound);
  }
  ...
  typename FloatTraits::value_type
  minimum_cycle_ratio(const Graph &g, VertexIndexMap vim,
                      EdgeWeight1Map ew1m, EdgeWeight2Map ew2m,  typename FloatTraits::value_type bound,
                      std::vector<typename graph_traits<Graph>::edge_descriptor> *pcc = 0,
                      FloatTraits = FloatTraits())
  {
      typedef detail::float_wrapper<
